...
class Grocery_CRUD_Generic_Model  extends Grocery_crud_model  {
    ...
    protected function relation_n_n_queries($select)
    {
        ...
        // $select = 'users.*, (SELECT GROUP_CONCAT(DISTINCT groups.name) FROM groups LEFT JOIN users_groups ON users_groups.group_id = groups.id WHERE users_groups.user_id = users.id GROUP BY users_groups.user_id) AS groups';
        $select = '*';
