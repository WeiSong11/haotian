...
class Grocery_crud_model  extends CI_Model  {
    ...
    function  get_total_price($query)
    {
        if($query == '')
            $arr = $this->db->query("SELECT SUM(ads_price) FROM {$this->table_name}")->result();
        else
            $arr = $this->db->query("SELECT SUM(ads_price) FROM {$this->table_name} WHERE {$query}")->result();
    ...
    function  get_total_result_advanced($query)
    {
        //$num_rows = $this->db->query('SELECT * FROM'.$this->table_name.$query)->num_rows();
        //$num_rows = $this->db->query("SHOW * FROM {$this->table_name } WHERE {$query}")->result()->num_rows();
        $arr  = $this->db->query("SELECT * FROM {$this->table_name} WHERE {$query} ")->result();

        return $num_rows  = count($arr);
    }
    function  get_query_advanced($query)
    {
        $arr  = $this->db->query("SELECT * FROM {$this->table_name} WHERE {$query} ORDER BY id ASC")->result();
        return $arr;
    }
    function  get_sum_price_advanced($query)
    {
        $arr  = $this->db->query("SELECT SUM(ads_price) FROM {$this->table_name} WHERE {$query}")->result();
        if($arr == null)
            return 0;
