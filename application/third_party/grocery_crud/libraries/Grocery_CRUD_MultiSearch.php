...
class Grocery_CRUD_MultiSearch extends Grocery_CRUD
{
    ...
    public $advanced_query_flag = false;
    public $temp_query = '';
...
//            "does not contains"     => "NOT LIKE"
        ...
        return array(
            ...
            "hidden_opts"=>$this->create_dropdown_opts($hidden_opts),
    ...
    protected function _update_data($data)
    {
        ...
        $property = array();
        $tmp      = array();
        $possible_opts = array("numeric_opts","string_opts","date_and_time","hidden_opts");
    ...
    protected function showList($ajax = false, $state_info = null)
    {
        ...
        $sum = 0;
        if ($this->get_table() == 'ads_manages') {
            foreach ($data->list as $one) {
                $sum += $one->ads_price;
            }
        }

        if($this->advanced_query_flag == false)
            $data->sumprice  =   $this->get_pricesum($this->temp_query);
        else
            $data->sumprice  =   $this->get_pricesum($this->temp_query);
        ...
        if(!$ajax)
        {
            ...
            $this->_theme_view('ads_list_template.php',$data);
        }
        else
        {
    ...
    protected function set_ajax_list_queries($state_info = null)
    {
        ...
        if($state_info->search_type == "advanced")
        {
            if($state_info->advanced_query_string != false)
            {
                ...
                $this->advanced_query_flag = true;
                $this->temp_query = $state_info->advanced_query_string;
                $this->where($state_info->advanced_query_string,NULL,FALSE);
                $this->where_advanced($state_info->advanced_query_string);
            }
            else {
                $state_info->advanced_query_string = false;
            }
...
// Existing Basic Search
    ...
    public function  showListInfo_Advanced($state_info = NULL)
    {
        if($state_info->search_type = 'advanced' && $state_info->advanced_query_string != false)
        {
                $total_results = $this->get_total_results_advanced($state_info->advanced_query_string);

                $sumprice = $this->get_pricesum($state_info->advanced_query_string);
                if($sumprice == null)   $sumprice = 0;

                //$sum_price = (int)get_pricesum_results_advanced($state_info->advanced_query_string);
                //$sum_price = $this->get_sum_price($state_info->advanced_query_string);
                @ob_end_clean();
                echo json_encode(array('total_results' => $total_results,'sumprice'=>$sumprice));
                die();
        }
        else
        {
             parent::showListInfo(); // TODO: Change the autogenerated stub
        }
    }
    public function get_list_advanced($query){
        ...
        $a = $this->basic_model->get_query_advanced($query);
        return $this->basic_model->get_query_advanced($query);
        //return $this->basic_model->db->query("SELECT * FROM ads_manages WHERE ads_user_id = '123456' ")->result();
        //return $this->basic_model->db->query("SELECT * FROM ads_manages WHERE {$query} ORDERBY id ASC")->result();
    }
    public function showList_Advanced($ajax,$state_info = NULL)
    {
        if($state_info->search_type = 'advanced' )
        {
            if($state_info->advanced_query_string != false)
            {
                ...
                $data->list = $this->get_list_advanced($state_info->advanced_query_string);
                ...
                $data->total_results = $this->get_total_results_advanced($state_info->advanced_query_string);
                $data->sumprice = $this->get_pricesum($state_info->advanced_query_string);
                if($data->sumprice == null) $data->sumprice = 0;
                $data->sumprice = $sum;
                $data->columns 				= $this->get_columns();
