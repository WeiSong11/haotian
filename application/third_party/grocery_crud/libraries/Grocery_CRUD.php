...
class grocery_CRUD_Model_Driver extends grocery_CRUD_Field_Types
{
	...
	protected function set_default_Model()
	{
        ...
        $ci->load->model('Grocery_crud_generic_model');
        $ci->load->model($model_name, $model_alias);
        $this->basic_model = $this->{$model_alias};*/

        $ci->load->model('Grocery_crud_model');
        $ci->load->model('Grocery_crud_generic_model');
        // $ci->load->model('Grocery_crud_model_MySQLi');
        $ci->load->model('Grocery_crud_model_Postgre');
        // $this->basic_model = new Grocery_crud_model_MySQLi();
    ...
    protected function get_pricesum($query)
    {
	    return $this->basic_model->get_total_price($query);
    ...
    protected function get_total_results_advanced($query = ''){
	    return $this->basic_model->get_total_result_advanced($query);
    }
    protected function get_pricesum_results_advanced($query = ''){
        return $this->basic_model->get_sum_price_advanced($query);
    }
	...
	// Commented by IlChol (2017.09.13)
	...
	protected function get_list()
	{
		...
		if(!empty($this->where))
			foreach($this->where as $where)
				// $this->basic_model->where($where[0],$where[1],$where[2]);	// Commented by IlChol (2017.09.13)
                $this->basic_model->where("ads_name","'ADS2'",$where[2]);	// Added by IlChol (2017.09.13)
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	public $multisearch = false;		// Added by IlChol (2017.09.13)
	const	VERSION = "1.5.8";
	...
	/* Added Variable */	// Added by IlChol (2017.09.13)
	protected $where_advanced       = array();
	...
	// Added by IlChol (2017.09.13), (start)
	public function where_advanced($key,$valu =NULL,$escape =TRUE)
    {
        $this->where_advanced = array($key);
	...
	public function render()
	{
		...
		switch ($this->state_code) {
			...
			case 7://ajax_list
				...
				//$this->showList(true);	// Commented by IlChol (2017.09.13)

				$this->showList_Advanced(true,$state_info);	// Added by IlChol (2017.09.13)
			...
			case 8://ajax_list_info
                ...
                //$this->showListInfo();	// Commented by IlChol (2017.09.13)
				$this->showListInfo_Advanced($state_info);	// Added by IlChol (2017.09.13)
			break;
